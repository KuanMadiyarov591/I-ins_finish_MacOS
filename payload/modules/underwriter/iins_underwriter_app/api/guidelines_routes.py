...
from iins_underwriter_app.services import guidelines_service as guidelines
...
def list_docs(user: User = Depends(require_underwriter)) -> Dict[str, Any]:
    return {"items": guidelines.list_guidelines(), "status": guidelines.status()}


@router.post("/guidelines/search")
def search_docs(body: SearchIn, user: User = Depends(require_underwriter)) -> Dict[str, Any]:
...
def get_doc(doc_id: str, user: User = Depends(require_underwriter)) -> Dict[str, Any]:
