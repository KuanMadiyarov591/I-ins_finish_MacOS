...
from iins_underwriter_app.services.vector_rag_store import VectorRagStore

_RAG_SYSTEM = (
    "Ты — цифровой помощник андеррайтера I-ins. "
    "Помогаешь оценивать риск, выбирать approve/refer/decline, проверять fraud-сигналы, "
    "авто- и моторные кейсы, продления и нормативные ограничения. "
    ...
    "Давай практичный следующий шаг андеррайтеру и перечисляй нужные доказательства. "
...
_DOMAIN_TERMS = {
    "андеррай", "страхов", "риск", "approve", "refer", "decline", "fraud",
    "мошеннич", "осаг", "дтп", "авто", "motor", "такси", "driver", "dui",
    "renew", "продлен", "убыт", "выплат", "преми", "полис", "коап", "ущерб",
}
_WORKFLOW_TERMS = {
    "approve", "refer", "decline", "fraud", "мошеннич", "dui", "police report",
    "такси", "renew", "продлен", "чеклист", "скор", "красн", "сигнал",
}
...
_ANSWER_LANG = {
    "ru": {
        "empty": (
            ...
            "Уточните линию бизнеса, риск-фактор, решение или нормативный вопрос."
        ),
        "intro": "По вопросу андеррайтера «{q}» в базе I-ins найдено:",
        "system_extra": "Отвечай на русском языке как риск-помощник андеррайтера.",
        "translate_to": "русский",
    },
    "kk": {
        "empty": (
            "I-ins андеррайтинг білім қорынан сенімді жауап беруге жеткілікті үзінді табылмады. "
            "Бизнес желісін, тәуекел факторын немесе шешімді нақтылаңыз."
        ),
        "intro": "Андеррайтердің «{q}» сұрағы бойынша I-ins базасынан табылды:",
        "system_extra": "Жауапты қазақ тілінде андеррайтерге арналған тәуекел әрекеті ретінде бер.",
        "translate_to": "қазақ",
    },
    "en": {
        "empty": (
            "No sufficiently relevant underwriting excerpts were found. "
            "Clarify the line of business, risk factor, decision, or regulatory issue."
        ),
        "intro": "For the underwriter question “{q}”, the I-ins knowledge base found:",
        "system_extra": "Answer in English with an underwriting action and required evidence.",
...
def _rewrite_answer_lang(answer: str, lang: str, question: str) -> Optional[str]:
    if lang == "ru":
        return None
    try:
        from iins_underwriter_app.services.llm_provider import (
            backend_ready,
            generate_reply,
            resolve_backend,
        )
        ...
        prompt = (
            f"Rewrite the underwriting answer below into {pack['translate_to']}. "
...
def status_payload() -> Dict[str, Any]:
    from iins_underwriter_app.services.llm_provider import providers_status, resolve_backend
    ...
    if ollama.get("model_ready"):
        ...
        msg = "База знаний андеррайтера пуста"
    else:
        msg = "PDF-база андеррайтера готова; Ollama недоступна"
...
def rag_query(
    ...
    *,
    ...
    from iins_underwriter_app.services.llm_provider import resolve_backend
