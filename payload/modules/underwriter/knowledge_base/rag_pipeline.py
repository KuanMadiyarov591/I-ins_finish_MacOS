"""Build and query the local underwriter I-ins PDF RAG corpus.
...
def _title_from_filename(path: Path) -> str:
    value = re.sub(
        r"^(?:01_\d+_client_and_02_insurance_agent_and_04_admin_|05_underwriter_internal_\d+_\d+_|underwriter_|underwr_|legal_)",
        "",
        path.stem,
        flags=re.IGNORECASE,
    )
...
def validate_retrieval() -> dict:
    cases = [
        {
            "query": "когда refer при DUI и нескольких ДТП",
            "expected_sources": ["05_underwriter_internal_01_"],
        },
        {
            "query": "сигналы мошенничества крупный ущерб без police report",
            "expected_sources": ["05_underwriter_internal_02_"],
        },
        {
            "query": "андеррайтинг такси возраст автомобиля история убытков",
            "expected_sources": ["05_underwriter_internal_03_"],
        },
        {
            "query": "возмещение вреда по ОСАГО порядок страховой выплаты",
            "expected_sources": ["legal_Федеральный", "underwr_Постановление"],
        },
        {
            "query": "необоснованный отказ от заключения публичного договора страхования КоАП",
            "expected_sources": ["underwriter_КоАП РФ Ст. 15.34.1"],
        },
    ]
    results = []
    for case in cases:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build and query the underwriter I-ins PDF RAG store")
