...
from iins_agent_app.services.vector_rag_store import VectorRagStore

_RAG_SYSTEM = (
    "Ты — цифровой помощник страхового агента I-ins. "
    "Помогаешь готовить звонки и follow-up, работать с возражениями, проверять "
    "условия продуктов и чеклист перед подачей заявки. "
    ...
    "Давай агенту конкретную формулировку для клиента и следующий шаг в CRM. "
...
_DOMAIN_TERMS = {
    "страхов", "полис", "агент", "клиент", "продаж", "возражен", "звон",
    "follow", "письм", "crm", "заявк", "чеклист", "преми", "покрыт", "договор",
    "каско", "дмс", "турист", "имуще", "жизн", "похорон", "бюджет", "скрипт",
}
_WORKFLOW_TERMS = {
    "возражен", "звон", "follow", "письм", "email", "crm", "продаж",
    "чеклист", "скрипт", "клиенту сказать", "подготовиться", "касани",
}
...
_ANSWER_LANG = {
    "ru": {
        "empty": (
            ...
            "Уточните продукт, возражение клиента или этап работы агента."
        ),
        "intro": "По вопросу агента «{q}» в базе I-ins найдено:",
        "system_extra": "Отвечай на русском языке как практический помощник страхового агента.",
        "translate_to": "русский",
    },
    "kk": {
        "empty": (
            "I-ins агенттік білім қорынан сенімді жауап беруге жеткілікті үзінді табылмады. "
            "Өнімді, клиент қарсылығын немесе агент әрекетін нақтылаңыз."
        ),
        "intro": "Агенттің «{q}» сұрағы бойынша I-ins базасынан табылды:",
        "system_extra": "Жауапты қазақ тілінде агентке арналған нақты әрекет ретінде бер.",
        "translate_to": "қазақ",
    },
    "en": {
        "empty": (
            "No sufficiently relevant insurance-agent excerpts were found. "
            "Clarify the product, client objection, or agent workflow."
        ),
        "intro": "For the agent question “{q}”, the I-ins knowledge base found:",
        "system_extra": "Answer in English with a client-ready phrase and the next CRM step.",
...
def _rewrite_answer_lang(answer: str, lang: str, question: str) -> Optional[str]:
    if lang == "ru":
        return None
    try:
        from iins_agent_app.services.llm_provider import (
            backend_ready,
            generate_reply,
            resolve_backend,
        )
        ...
        prompt = (
            f"Rewrite the insurance agent answer below into {pack['translate_to']}. "
...
def status_payload() -> Dict[str, Any]:
    from iins_agent_app.services.llm_provider import providers_status, resolve_backend
    ...
    if ollama.get("model_ready"):
        ...
        msg = "База знаний страхового агента пуста"
    else:
        msg = "PDF-база страхового агента готова; Ollama недоступна"
...
def rag_query(
    ...
    *,
    ...
    from iins_agent_app.services.llm_provider import resolve_backend
