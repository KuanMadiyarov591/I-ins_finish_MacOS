...
def backend_ready(backend: str) -> bool:
    ...
    if backend == "ollama":
        try:
            from iins_agent_app.services.ollama_lm import ollama_is_ready
    ...
    if backend == "gigachat":
        try:
            from iins_agent_app.services.gigachat_lm import gigachat_is_ready
...
def not_ready_message(backend: str) -> str:
    ...
    if backend == "gigachat":
        try:
            from iins_agent_app.services.gigachat_lm import gigachat_status
...
def generate_reply(
    ...
    *,
    ...
    if backend == "gigachat":
        from iins_agent_app.services.gigachat_lm import generate_gigachat_reply
    ...
    from iins_agent_app.services.ollama_lm import generate_ollama_reply
...
def providers_status() -> dict[str, Any]:
    ...
    try:
        from iins_agent_app.services.ollama_lm import ollama_status
