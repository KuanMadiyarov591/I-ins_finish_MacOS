"""Build and query the local insurance-agent I-ins PDF RAG corpus.
...
def _title_from_filename(path: Path) -> str:
    value = re.sub(
        r"^(?:01_\d+_client_and_02_insurance_agent_and_04_admin_|02_agent_internal_\d+_\d+_)",
        "",
        path.stem,
        flags=re.IGNORECASE,
    )
...
def validate_retrieval() -> dict:
    cases = [
        {
            "query": "что ответить клиенту на возражение дорого нет бюджета",
            "expected_sources": ["02_agent_internal_05_"],
        },
        {
            "query": "чеклист агента перед подачей страховой заявки",
            "expected_sources": ["02_agent_internal_04_", "02_agent_internal_06_"],
        },
        {
            "query": "как подготовиться к звонку и назначить follow-up",
            "expected_sources": ["02_agent_internal_07_", "02_agent_internal_05_"],
        },
        {
            "query": "права клиента при заключении договора страхования",
            "expected_sources": ["01_01_", "01_03_", "01_04_", "01_05_"],
        },
        {
            "query": "согласие клиента на обработку персональных данных",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build and query the insurance-agent I-ins PDF RAG store")
