...
from iins_client_app.services.vector_rag_store import VectorRagStore

_RAG_SYSTEM = (
    "Ты — цифровой помощник клиента страховой компании I-ins. "
    "Помогаешь понять страховые продукты, порядок обращения, права потребителя, "
    "сроки, исключения и требования к документам. "
    ...
    "Давай практичный и понятный следующий шаг клиенту. "
...
_DOMAIN_TERMS = {
    "страхов", "полис", "выплат", "преми", "договор", "клиент", "потребител",
    "омбудсман", "персональн", "каско", "дмс", "турист", "имуще", "жизн",
    "возмещ", "случа", "заявлен", "исключен", "франшиз",
}
...
_ANSWER_LANG = {
    "ru": {
        "empty": (
            ...
            "Уточните вопрос по полису, страховому случаю или правам потребителя."
        ),
        "intro": "По вопросу «{q}» в базе I-ins найдено:",
        "system_extra": "Отвечай на русском языке простыми словами и предложи клиенту следующий шаг.",
        "translate_to": "русский",
    },
    "kk": {
        "empty": (
            "I-ins білім қорынан сенімді жауап беруге жеткілікті үзінді табылмады. "
            "Полис, сақтандыру жағдайы немесе тұтынушы құқығы туралы сұрақты нақтылаңыз."
        ),
        "intro": "«{q}» сұрағы бойынша I-ins базасынан табылды:",
        "system_extra": "Жауапты қазақ тілінде, клиентке түсінікті етіп бер.",
        "translate_to": "қазақ",
    },
    "en": {
        "empty": (
            "No sufficiently relevant I-ins knowledge excerpts were found. "
            "Clarify the policy, claim, or consumer-rights question."
        ),
        "intro": "For the client question “{q}”, the I-ins knowledge base found:",
        "system_extra": "Answer in plain English and give the client a practical next step.",
...
def _rewrite_answer_lang(answer: str, lang: str, question: str) -> Optional[str]:
    if lang == "ru":
        return None
    try:
        from iins_client_app.services.llm_provider import (
            backend_ready,
            generate_reply,
            resolve_backend,
        )
        ...
        prompt = (
            f"Rewrite the insurance client answer below into {pack['translate_to']}. "
...
def status_payload() -> Dict[str, Any]:
    from iins_client_app.services.llm_provider import providers_status, resolve_backend
    ...
    if ollama.get("model_ready"):
        ...
        msg = "Клиентская база знаний пуста"
    else:
        msg = "Клиентская PDF-база готова; Ollama недоступна"
...
def rag_query(
    ...
    *,
    ...
    from iins_client_app.services.llm_provider import resolve_backend
