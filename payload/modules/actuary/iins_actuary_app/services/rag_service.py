...
from iins_actuary_app.services.vector_rag_store import VectorRagStore

_RAG_SYSTEM = (
    "Ты — цифровой консультант актуария Actuary Desk. "
    "Помогаешь: тарификацию, CGR, территории, leakage и прогноз selected_premium, "
    "рейтинговые допущения, CGR-факторы и территориальные определения. "
    ...
    "Давай практичный совет актуарию (что проверить / какое решение обосновать). "
    "1–4 коротких абзаца. Подтверждай факты ссылками вида [1], [2] на предоставленные источники."
)

_KK_LETTERS = set("әғқңөұүһіӘҒҚҢӨҰҮҺІ")

# Conservative grounding thresholds. A query without explicit actuarial vocabulary
# must have stronger textual evidence before it is allowed to reach the LLM.
_MIN_RELEVANCE_SCORE = 0.18
_MIN_DOMAIN_RELEVANCE_SCORE = 0.14
_DOMAIN_TERMS = {
    "актуар", "страхов", "перестрахов", "резерв", "преми", "тариф",
    "платежеспособ", "финансов", "нпф", "пенсион", "cgr", "territory",
    "leakage", "selected_premium", "рейтинг", "допущен", "убыточ",
}
...
_ANSWER_LANG = {
    "ru": {
        "empty": (
            ...
            "Уточните вопрос по актуарной деятельности либо добавьте профильный документ в базу."
        ),
        "intro": "По вопросу «{q}» для актуария найдено:",
        "system_extra": "Отвечай на русском языке. Формат: совет актуарию Actuary Desk.",
        "translate_to": "русский",
    },
    "kk": {
        "empty": (
            "Actuary Desk білім қорынан сәйкес үзінді табылмады. "
            "CGR / territory немесе leakage нақтылаңыз."
        ),
        "intro": "«{q}» сұрағы бойынша актуарийге мыналар табылды:",
        "system_extra": "Жауапты қазақ тілінде бер. Бұл кеңес Actuary Desk актуарийіне.",
        "translate_to": "қазақ",
    },
    "en": {
        "empty": (
            "No relevant Actuary Desk knowledge excerpts were found. "
            "Clarify the CGR, territory, or leakage."
        ),
        "intro": "For the actuary question “{q}”, found:",
        "system_extra": "Answer in English. Format as advice for a Actuary Desk actuary.",
...
def _rewrite_answer_lang(answer: str, lang: str, question: str) -> Optional[str]:
    ...
    try:
        ...
        prompt = (
            f"Rewrite the legal advisor answer below into {pack['translate_to']}. "
...
def status_payload() -> Dict[str, Any]:
    from iins_actuary_app.services.llm_provider import providers_status, resolve_backend
...
def rag_query(
    ...
    *,
    ...
    from iins_actuary_app.services.llm_provider import resolve_backend
