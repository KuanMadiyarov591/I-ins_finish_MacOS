...
def require_actuary(user: User = Depends(get_current_user)) -> User:
    if user.role not in ("actuary", "admin"):
        raise HTTPException(status_code=403, detail="Только для актуария")
    return user


# Compat aliases used by sibling desks
require_lawyer = require_actuary
require_underwriter = require_actuary
