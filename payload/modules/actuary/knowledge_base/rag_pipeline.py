"""Build and query a local RAG store from the actuarial PDF corpus.
...
def _title_from_filename(path: Path) -> str:
    value = re.sub(r"^03_\d+_Actuary_?", "", path.stem, flags=re.IGNORECASE)
...
def validate_retrieval() -> dict:
    cases = [
        {
            "query": "требования к актуарному заключению страховщика",
            "expected_sources": ["03_34_", "03_21_"],
        },
        {
            "query": "финансовая устойчивость и платежеспособность страховщика",
            "expected_sources": ["03_33_", "03_20_"],
        },
        {
            "query": "используемые предположения при расчете страховых резервов",
            "expected_sources": ["03_27_", "03_26_"],
        },
        {
            "query": "актуарный дефицит негосударственного пенсионного фонда",
            "expected_sources": ["03_39_"],
        },
        {
            "query": "актуарное оценивание обязательств негосударственного пенсионного фонда",
            "expected_sources": ["03_24_", "03_35_"],
        },
    ]
    results = []
    for case in cases:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build and query the actuarial PDF RAG store")
