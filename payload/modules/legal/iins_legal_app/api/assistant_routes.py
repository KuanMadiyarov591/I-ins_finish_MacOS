...
from iins_legal_app.services import rag_service
...
def rag_status(_: User = Depends(require_lawyer)) -> Dict[str, Any]:
    return rag_service.status_payload()


@router.post("/ask")
def assistant_ask(body: AskIn, _: User = Depends(require_lawyer)) -> Dict[str, Any]:
...
def rag_reload(_: User = Depends(require_lawyer)) -> Dict[str, Any]:
    idx = rag_service.reload_index()
    return {"ok": True, "chunks": len(idx.chunks)}


@router.get("/rag/document/{source}")
def rag_document(source: str, _: User = Depends(require_lawyer)) -> FileResponse:
