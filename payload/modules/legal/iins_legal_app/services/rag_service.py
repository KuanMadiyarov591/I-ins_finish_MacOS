...
from iins_legal_app.services.vector_rag_store import VectorRagStore

_RAG_SYSTEM = (
    "Ты — цифровой помощник юриста страховой компании I-ins. "
    "Помогаешь разбирать страховые претензии, PI/IMR-кейсы, NDA, досудебный и судебный порядок. "
    "Отвечай строго на основе блока «Контекст из базы знаний». "
    "Если в контексте нет данных — скажи об этом явно. "
    "Не выдумывай нормы, сроки, суммы и реквизиты вне контекста. "
    "Разделяй факт из источника и рабочую рекомендацию; указывай следующий шаг и нужные доказательства. "
    "Для юридически значимого решения требуй проверку актуальной официальной редакции. "
...
_DOMAIN_TERMS = {
    "юрист", "прав", "закон", "кодекс", "суд", "иск", "претенз", "жалоб", "апелляц",
    "страхов", "осаг", "дтп", "выплат", "возмещ", "полис", "договор", "коап", "ущерб",
    "pi", "imr", "nda", "claim", "конфиденц", "персональн", "потребител", "подсуд",
    "арбитраж", "гпк", "апк", "омбудсмен", "финансов", "доказательств", "срок",
}
_WORKFLOW_TERMS = {
    "pi", "imr", "nda", "claim", "претенз", "апелляц", "appeal", "intake",
    "эскалац", "документ", "доказательств", "конфиденц", "согласован", "чеклист",
}

_ANSWER_LANG = {
    "ru": {
        "empty": (
            "В юридической базе знаний нет достаточно релевантной информации для достоверного ответа. "
            "Уточните вид спора, участника, документ, событие и требуемое процессуальное действие."
        ),
        "intro": "По юридическому вопросу «{q}» в базе I-ins найдено:",
        "system_extra": "Отвечай на русском языке как помощник юриста страховой компании.",
        "translate_to": "русский",
    },
    "kk": {
        "empty": (
            "I-ins заң білім қорынан сенімді жауап беруге жеткілікті үзінді табылмады. "
            "Даудың түрін, құжатты және қажетті процестік әрекетті нақтылаңыз."
        ),
        "intro": "Андеррайтердің «{q}» сұрағы бойынша I-ins базасынан табылды:",
        "system_extra": "Жауапты қазақ тілінде сақтандыру компаниясының заңгеріне арналған әрекет ретінде бер.",
        "translate_to": "қазақ",
    },
    "en": {
        "empty": (
            "No sufficiently relevant legal excerpts were found. "
            "Clarify the dispute, party, document, event, and required procedural action."
        ),
        "intro": "For the legal question “{q}”, the I-ins knowledge base found:",
        "system_extra": "Answer in English with a legal next step and required evidence.",
...
def _grounded_hits(query: str, hits: List[Tuple[_Chunk, float]]) -> List[Tuple[_Chunk, float]]:
    ...
    if hits[0][1] < threshold:
        return []
    grounded = [(chunk, score) for chunk, score in hits if score >= max(threshold * 0.72, hits[0][1] * 0.42)]
    normalized = query.lower().replace("ё", "е")
    if "_internal_" in hits[0][0].source and any(term in normalized for term in _WORKFLOW_TERMS):
        grounded = [(chunk, score) for chunk, score in grounded if "_internal_" in chunk.source]
    return grounded


def _route_legal_candidates(query: str, hits: List[Tuple[_Chunk, float]]) -> List[Tuple[_Chunk, float]]:
    """Prioritize the matching internal playbook for explicit PI/IMR/NDA/intake workflows."""
    normalized = query.lower().replace("ё", "е")
    routes = (
        (r"\bpi\b", "_pi_claims_"),
        (r"\bimr\b", "_imr_appeals_"),
        (r"\bnda\b", "_nda_notes"),
        (r"\bintake\b|регистрац|первичн.*прием", "_claims_intake"),
    )
    routed: List[Tuple[_Chunk, float]] = []
    for chunk, score in hits:
        source = chunk.source.lower()
        boosted = score
        if any(re.search(pattern, normalized) and marker in source for pattern, marker in routes):
            boosted *= 3.5
        routed.append((chunk, boosted))
...
def retrieve(query: str, top_k: int = 4) -> List[Tuple[_Chunk, float]]:
    ...
    if store is not None:
        candidates = [
            ...
            for hit in store.search(query, top_k=max(64, top_k * 16))
        ]
        return _grounded_hits(query, _route_legal_candidates(query, candidates)[:top_k])
    idx = get_index()
    if not idx.chunks:
        return []
    q = query.strip()
...
def _rewrite_answer_lang(answer: str, lang: str, question: str) -> Optional[str]:
    if lang == "ru":
        return None
    try:
        from iins_legal_app.services.llm_provider import (
            backend_ready,
            generate_reply,
            resolve_backend,
        )
        ...
        prompt = (
            f"Rewrite the legal answer below into {pack['translate_to']}. "
...
def status_payload() -> Dict[str, Any]:
    from iins_legal_app.services.llm_provider import providers_status, resolve_backend
    ...
    if ollama.get("model_ready"):
        ...
        msg = "Юридическая база знаний пуста"
    else:
        msg = "Юридическая PDF-база готова; Ollama недоступна"
...
def rag_query(
    ...
    *,
    ...
    from iins_legal_app.services.llm_provider import resolve_backend
