...
def require_lawyer(user: User = Depends(get_current_user)) -> User:
    if user.role not in ("lawyer", "admin"):
        raise HTTPException(status_code=403, detail="Только для юриста")
    return user


# Alias for desks that still import UW-style name
require_underwriter = require_lawyer
