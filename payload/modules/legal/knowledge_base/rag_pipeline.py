"""Build and query the local legal I-ins PDF RAG corpus.
...
def _title_from_filename(path: Path) -> str:
    value = re.sub(
        r"^(?:01_\d+_client_and_02_insurance_agent_and_04_admin_|06_legal_internal_\d+_\d+_|legal_|underwriter_|underwr_)",
        "",
        path.stem,
        flags=re.IGNORECASE,
    )
...
def _normalize_page_text(text: str) -> str:
    ...
    normalized = re.sub(
        r"Бесплатная юридическая консультация по телефонам:.*?(?:https?://)?(?:gpkodeksrf|apkodrf)\.ru/?",
        " ",
        normalized,
        flags=re.IGNORECASE | re.DOTALL,
    )
    normalized = re.sub(r"(?:https?://)?(?:gpkodeksrf|apkodrf)\.ru/?", " ", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"(?:\+?7|8)\s*\(?\d{3}\)?(?:[\s-]*\d){7}", " ", normalized)
...
def search(query: str, top_k: int = 5) -> list[dict]:
    ...
    for row in rows:
        ...
        if any(re.search(pattern, query_lower) and marker in source_lower for pattern, marker in role_routes):
            score *= 3.5
        if score > 0:
...
def validate_retrieval() -> dict:
    cases = [
        {
            "query": "PI претензия тяжкий вред жизни эскалация и доказательства",
            "expected_sources": ["06_legal_internal_01_"],
        },
        {
            "query": "IMR апелляция на отказ сроки медицинские документы",
            "expected_sources": ["06_legal_internal_02_"],
        },
        {
            "query": "NDA конфиденциальная информация обязательства и исключения",
            "expected_sources": ["06_legal_internal_04_"],
        },
        {
            "query": "подсудность спора физического лица со страховой компанией",
            "expected_sources": ["legal_gpkodeksrf", "01_05_"],
        },
        {
            "query": "арбитражный спор юридического лица со страховщиком иск",
            "expected_sources": ["legal_apk"],
        },
        {
            "query": "ОСАГО досудебный порядок страховое возмещение потерпевшему",
            "expected_sources": ["legal_Федеральный", "underwr_Постановление"],
        },
    ]
    results = []
    for case in cases:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build and query the legal I-ins PDF RAG store")
